...
sums :: Int -> [Int]
sums i = kernel 0 1
  where kernel total n =
          let total' = total + n
          in if n == i then [total']
                       else total' : kernel total' (n+1)
...
mymaximum bigger initial (x:xs) = mymaximum bigger biggest xs
  where biggest = if x `bigger` initial then x else initial
