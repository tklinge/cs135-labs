...
totalPrice (MkShoppingEntry nm pr ct) = pr *  fromIntegral ct
...
buyOneMore (MkShoppingEntry nm pr ct) = MkShoppingEntry nm pr (ct + 1)
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = [(r1+r2)/2, (g1+g2)/2, (b1+b2)/2]
  where [r1,g1,b1] = rgb c1
        [r2,g2,b2] = rgb c2
rgb (Invert c) = [1-r, 1-g, 1-b]
  where [r,g,b] = rgb c
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne p) = 1 + fromNat p

toNat :: Int -> Maybe Nat
toNat n
  | n < 0     = Nothing
  | n == 0    = Just Zero
  | otherwise = Just (PlusOne p)
      where Just p = toNat (n-1)
...
prettyPrint = helper ""
  where helper s End = s
        helper s (O b) = helper ('0' : s) b
        helper s (I b) = helper ('1' : s) b
