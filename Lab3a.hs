...
firstHalf s = let n  = length s
                  n2 = n `div` 2
              in if even n then take n2 s
                           else take (n2 + 1) s
...
joinToLength n set = filter ((==n) . length) [x ++ y | x <- set, y <- set]
...
------------------------------------------------------------------------------
-- Ex 11: remember the lectureParticipants example from the reading? We
