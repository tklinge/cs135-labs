...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node x lft rgt) = 1 + treeSize lft + treeSize rgt
...
treeMax Empty = 0
treeMax (Node x lft rgt) = maximum [x, treeMax lft, treeMax rgt]
...
allValues f (Node x lft rgt) = f x && allValues f lft && allValues f rgt
...
isOrdered (Node x l r) = allValues (<=x) l && allValues (>=x) r
...
set (x:xs) val (Node y l r) = case x of
  StepL -> Node y (set xs val l) r
  StepR -> Node y l (set xs val r)
...
search val (Node x l r)
  | val == x  = Just []
  | otherwise = case (search val l, search val r) of
      (Just xs, _) -> Just (StepL:xs)
      (_, Just xs) -> Just (StepR:xs)
      (_,_)        -> Nothing
